...
beforeEach(() => {
    stores.books.searchResults = [BOOK_ITEM_1, BOOK_ITEM_2, BOOK_ITEM_WITHOUT_CATEGORIES];
});
...
describe('Mobx Stores', () => {
    ...
    it("update categories list after item was added", () => {
        ...
        expect(Object.keys(stores.books.categories) ).toContain(BOOK_ITEM_1.categories[0]);
        expect(Object.keys(stores.books.categories) ).toContain(BOOK_ITEM_2.categories[0]);
    });
    ...
    it("activeItems should only display current category elements", () => {
        ...
        expect(stores.books.activeItems[0].title).toBe(BOOK_ITEM_WITHOUT_CATEGORIES.title);
    });
});